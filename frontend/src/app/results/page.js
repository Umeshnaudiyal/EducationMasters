...
export default function ResultsPage() {
  ...
  const fetchResults = async (pageNum) => {
    try {
      setLoading(true);
      // 1. Fetch from dedicated results endpoint
      const res = await fetch(`${API_BASE}/results?status=publish&page=${pageNum}&limit=${LIMIT}`);
      ...
      if (data.success && data.data && data.data.length > 0) {
        ...
        // 2. Fallback to blogs with type result
        const fallbackRes = await fetch(`${API_BASE}/blogs?type=result&page=${pageNum}&limit=${LIMIT}`);
          ...
          <main className="col-span-1 lg:col-span-8 xl:col-span-7 space-y-4">
            ...
            <div className="border-b border-slate-200 pb-3">
              <h1 className="text-2xl sm:text-3xl font-bold text-slate-900 tracking-tight">
                Latest Government Exam Results &amp; Cut-off 2026
              </h1>
              <p className="text-xs sm:text-sm text-slate-500 mt-1">
                Check official merit lists, score cards, cut-off marks, and selection lists for all central &amp; state exams.
              </p>
            </div>
            ...
            {loading ? (
              ...
              <div className="bg-slate-50 border border-slate-200 rounded-xl p-8 text-center space-y-3">
                ...
                <p className="text-xs text-slate-500">Check back soon for latest declared exam results and merit lists.</p>
              </div>
            ) : (
                      ...
                      <div className="flex-1 flex flex-col justify-between">
                        <div>
                          ...
                          <div className="flex flex-wrap items-center gap-x-1.5 gap-y-1 text-xs text-slate-500 font-medium mt-1.5">
                            ...
                            <span>|</span>
                            <span>In <strong className="text-slate-800 font-medium">Result</strong></span>
                            <span>|</span>
                          ...
                          <p className="text-xs sm:text-sm text-slate-600 mt-2.5 line-clamp-2 leading-relaxed">
                            {item.description?.replace(/<[^>]*>?/gm, '') || `Check official exam result, answer key, merit list, and cut-off for ${item.title}.`}
                          </p>
                        </div>

                        {/* Bottom Tag - Pure Result Category / Department */}
                        <div className="mt-4 text-xs text-slate-700 font-medium flex items-center justify-between">
                          <div>
                            Category: <span className="text-blue-600 font-semibold">Result</span>
                          </div>
