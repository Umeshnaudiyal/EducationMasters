...
export default function ResultsPage() {
  ...
  const fetchResults = async (pageNum) => {
    try {
      setLoading(true);
      const res = await fetch(`${API_BASE}/blogs?type=result&page=${pageNum}&limit=${LIMIT}`);
                      ...
                      <div className="flex-1 flex flex-col justify-between">
                        <div>
                          ...
                          <div className="flex flex-wrap items-center gap-x-1.5 gap-y-1 text-xs text-slate-500 font-medium mt-1">
                            ...
                            <span>|</span>
                            <span>In <strong className="text-slate-800 font-medium">{item.categories?.[0]?.name || 'Results'}</strong></span>
                            <span>|</span>
                          ...
                          <p className="text-xs sm:text-sm text-slate-600 mt-2.5 line-clamp-2 leading-relaxed">
                            {item.description?.replace(/<[^>]*>?/gm, '') || `Check official exam result, answer key, and merit list for ${item.title}.`}
                          </p>
                        </div>

                        {/* Bottom Tag */}
                        <div className="mt-4 text-xs text-slate-700 font-medium">
                          Category: <span className="text-blue-600 font-semibold">{item.categories?.[0]?.name || 'Exam Results'}</span>
                        </div>
                      </div>
                    </article>
                  );
